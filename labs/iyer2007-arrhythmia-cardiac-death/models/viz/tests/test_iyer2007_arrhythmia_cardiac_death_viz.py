...
"""Tests for the Iyer2007 Arrhythmia Cardiac Death visualization sub-model. No SBML or
...
from src.iyer2007_arrhythmia_cardiac_death_viz import Iyer2007ArrhythmiaCardiacdeathModel1006230030ModelViz  # noqa: E402
...
def test_instantiation():
    m = Iyer2007ArrhythmiaCardiacdeathModel1006230030ModelViz()
...
def test_lab_question_threads_through_visuals():
    m = Iyer2007ArrhythmiaCardiacdeathModel1006230030ModelViz(lab_question="Does this experiment work?")
    state = _record("state", {"x": 1.0})
    m.advance_window(0.0, 0.5, inputs={"state": state})
    visuals = m.visualize()
...
def test_generic_title_when_lab_question_blank():
    m = Iyer2007ArrhythmiaCardiacdeathModel1006230030ModelViz(lab_question="")
    state = _record("state", {"x": 1.0})
    m.advance_window(0.0, 0.5, inputs={"state": state})
    visuals = m.visualize()
    assert visuals[0]["data"]["title"] == "What did the model simulate?"

...
The captured run applies the configured pressure-load challenge, changing arterial pressure from `103.525` to `124.23` during the challenge window before returning to baseline. It evaluates 600 s of simulated dynamics across Heart-strength lower state, Heart-strength response, Heart-muscle mass deficit; read the absolute trajectories, baseline-relative plot, response ranking, and summary table together because the variables use different native scales.

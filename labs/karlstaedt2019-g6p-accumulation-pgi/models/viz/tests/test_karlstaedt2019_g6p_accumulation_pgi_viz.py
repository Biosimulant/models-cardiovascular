...
"""Tests for the Karlstaedt2019 G6p Accumulation Pgi visualization sub-model. No SBML or
...
from src.karlstaedt2019_g6p_accumulation_pgi_viz import Karlstaedt2019G6pAccumulationViaPhosphoglucoModel1910170001ModelViz  # noqa: E402
...
def test_instantiation():
    m = Karlstaedt2019G6pAccumulationViaPhosphoglucoModel1910170001ModelViz()
...
def test_lab_question_threads_through_visuals():
    m = Karlstaedt2019G6pAccumulationViaPhosphoglucoModel1910170001ModelViz(lab_question="Does this experiment work?")
    state = _record("state", {"x": 1.0})
    m.advance_window(0.0, 0.5, inputs={"state": state})
    visuals = m.visualize()
...
def test_generic_title_when_lab_question_blank():
    m = Karlstaedt2019G6pAccumulationViaPhosphoglucoModel1910170001ModelViz(lab_question="")
    state = _record("state", {"x": 1.0})
    m.advance_window(0.0, 0.5, inputs={"state": state})
    visuals = m.visualize()
    assert visuals[0]["data"]["title"] == "What did the model simulate?"

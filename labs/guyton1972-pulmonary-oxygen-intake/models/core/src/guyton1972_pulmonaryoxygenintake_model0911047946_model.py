...
"""Auto-generated SBML BioModule wrapper for Guyton1972_PulmonaryOxygenIntake.
...
class SbmlGuyton1972Pulmonaryoxygenintake(biosim.BioModule):
    """BioModule wrapper for SBML model: Guyton1972_PulmonaryOxygenIntake."""

    _PORT_TO_SBML: dict[str, str] = {
        "pulmonary_blood_flow": "QRO",
        "oxygen_demand": "RMO",
    ...
    def __init__(self, model_path: str = "data/MODEL0911047946.xml", integration_step: float = 0.01) -> None:
    ...
    def inputs(self) -> dict[str, SignalSpec]:
        return {
            "pulmonary_blood_flow": SignalSpec.scalar(
                dtype="float64",
                accepted_profiles=(
                    AcceptedSignalProfile(signal_type="scalar", dtype="float64", accepted_units=("dimensionless",)),
                    AcceptedSignalProfile(signal_type="record", schema={"payload": "json"}),
                ),
                description="Sets pulmonary blood flow for the simulation scenario. Default `4.97838`.",
            ),
            "oxygen_demand": SignalSpec.scalar(
                dtype="float64",
                accepted_profiles=(
                    AcceptedSignalProfile(signal_type="scalar", dtype="float64", accepted_units=("dimensionless",)),
                    AcceptedSignalProfile(signal_type="record", schema={"payload": "json"}),
                ),
                description="Sets oxygen demand for the simulation scenario. Default `56.8057`.",
            ),
            "ambient_oxygen": SignalSpec.scalar(
                dtype="float64",
                accepted_profiles=(
                    AcceptedSignalProfile(signal_type="scalar", dtype="float64", accepted_units=("dimensionless",)),
                    AcceptedSignalProfile(signal_type="record", schema={"payload": "json"}),
                ),
                description="Sets ambient oxygen for the simulation scenario. Default `150.0`.",
            ),

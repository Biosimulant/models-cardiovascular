# Guyton1972 Muscle Blood Flow Lab

This lab asks: **Does active muscle receive more blood flow?**
...
- Muscle blood-flow response
- Fast muscle-flow response
- Delayed muscle-flow response
- Local muscle-flow adaptation

The captured run applies the configured oxygen availability challenge, changing muscle oxygen from `38.0666` to `19.0333` during the challenge window before returning to baseline. It evaluates 600 s of simulated dynamics across Muscle blood-flow response, Fast muscle-flow response, Delayed muscle-flow response, Local muscle-flow adaptation; read the absolute trajectories, baseline-relative plot, response ranking, and summary table together because the variables use different native scales.
...
1. `guyton1972_muscle_blood_flow_core`: SBML simulator. Receives every contextual input port (ion concentrations, voltages, conductances, etc. — see below), drives roadrunner, and emits two outputs: `state` (per-window species/state-variable record) and `species_labels` (a one-time map of `{species_id: human_label}` lifted from the SBML `<species name=...>` attributes).
2. `guyton1972_muscle_blood_flow_viz`: visualization sub-model. Consumes both `state` and `species_labels` from the core, accumulates the per-window history, prettifies the labels, and renders the two visuals plus the `run_summary` output. No SBML, no roadrunner — pure presentation.
...
- `models/core/src/guyton1972_musclebloodflowcontrol_model0911202318_model.py` is the SBML wrapper (no visualization code).
- `models/core/data/MODEL0911202318.xml` is the original SBML file from BioModels.
...
- `models/viz/src/guyton1972_muscle_blood_flow_viz.py` consumes core state + labels and renders timeseries, Q&A table, and the `run_summary` record.
...
|---|---|---|---|
| `muscle_oxygen` | Sets muscle oxygen for the scenario. | 38.0666 | — |
...
```bash
biosimulant labs import labs/guyton1972-muscle-blood-flow
```
...
- License: `CC0` (from upstream BioModels entry [biomodels_ebi:MODEL0911202318](https://www.ebi.ac.uk/biomodels/MODEL0911202318)).
- This wrapper does not modify the upstream biology. To change rates, initial conditions, or kinetic laws, edit the SBML file in `models/core/data/MODEL0911202318.xml` directly.

...
"""Auto-generated SBML BioModule wrapper for Guyton1972_Electrolytes.
...
class SbmlGuyton1972Electrolytes(biosim.BioModule):
    """BioModule wrapper for SBML model: Guyton1972_Electrolytes."""
    ...
    def __init__(self, model_path: str = "data/MODEL0912160001.xml", integration_step: float = 0.01) -> None:
    ...
    def inputs(self) -> dict[str, SignalSpec]:
        return {
            "sodium_intake": SignalSpec.scalar(
                dtype="float64",
                accepted_profiles=(
                    AcceptedSignalProfile(signal_type="scalar", dtype="float64", accepted_units=("dimensionless",)),
                    AcceptedSignalProfile(signal_type="record", schema={"payload": "json"}),
                ),
                description="Sets sodium intake for the simulation scenario. Default `0.1`.",
            ),
            "potassium_intake": SignalSpec.scalar(
                dtype="float64",
                accepted_profiles=(
                    AcceptedSignalProfile(signal_type="scalar", dtype="float64", accepted_units=("dimensionless",)),
                    AcceptedSignalProfile(signal_type="record", schema={"payload": "json"}),
                ),
                description="Sets potassium intake for the simulation scenario. Default `0.08`.",
            ),
            "aldosterone_drive": SignalSpec.scalar(
                dtype="float64",
                accepted_profiles=(
                    AcceptedSignalProfile(signal_type="scalar", dtype="float64", accepted_units=("dimensionless",)),
                    AcceptedSignalProfile(signal_type="record", schema={"payload": "json"}),
                ),
                description="Sets aldosterone drive for the simulation scenario. Default `1.037`.",
            ),

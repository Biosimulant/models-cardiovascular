# Pasek2006 Ventricular Cardiomyocytes Lab

This lab asks: **How do T-tubule conditions affect ventricular excitation?**

The lab is composed of two sub-models: a `core` SBML simulator (encoded as SBML, solved by Tellurium) and a dedicated `viz` presenter that turns the raw simulation state into a friendly timeseries plot and a plain-language "What Happened" summary. The split keeps the SBML wrapper clean and makes it easy to swap or extend the visualization without touching the simulator. This a model from the article: The functional role of cardiac T-tubules explored in a model of rat ventricular myocytes. It can be used to explore cardiac dynamics and compare response patterns across conditions.

## What You'll See

The lab opens as a canvas with three model nodes wired in series: the scenario driver on the left, the core simulator in the middle, and the visualization sub-model on the right. After running, the viz node emits four visualizations: an event-annotated absolute timeseries, a baseline-relative response timeseries, a signed response bar chart, and a question-and-answer table titled `What Happened` that answers `How do T-tubule conditions affect ventricular excitation?` in plain language. The viz node also publishes the table as a structured `run_summary` record that downstream nodes can consume.
...
The captured run applies the configured calcium activation challenge, changing extracellular calcium from `1.2` to `2.4` during the challenge window before returning to baseline. It evaluates 1 s of simulated dynamics across T-tubule volume, Deep cytosol volume, Surface-membrane capacitance, T-tubule membrane capacitance; read the absolute trajectories, baseline-relative plot, response ranking, and summary table together because the variables use different native scales.
...
1. `pasek2006_ventricular_cardiomyocytes_core`: SBML simulator. Receives every contextual input port (ion concentrations, voltages, conductances, etc. — see below), drives roadrunner, and emits two outputs: `state` (per-window species/state-variable record) and `species_labels` (a one-time map of `{species_id: human_label}` lifted from the SBML `<species name=...>` attributes).
2. `pasek2006_ventricular_cardiomyocytes_viz`: visualization sub-model. Consumes both `state` and `species_labels` from the core, accumulates the per-window history, prettifies the labels, and renders the two visuals plus the `run_summary` output. No SBML, no roadrunner — pure presentation.
...
- `models/core/src/pasek2006_ventricularcardiomyocytes_model0406553884_model.py` is the SBML wrapper (no visualization code).
- `models/core/data/MODEL0406553884.xml` is the original SBML file from BioModels.
...
- `models/viz/src/pasek2006_ventricular_cardiomyocytes_viz.py` consumes core state + labels and renders timeseries, Q&A table, and the `run_summary` record.
...
|---|---|---|---|
| `extracellular_sodium` | Sets extracellular sodium for the scenario. | 140 | mM |
| `extracellular_potassium` | Sets extracellular potassium for the scenario. | 5.4 | mM |
| `extracellular_calcium` | Sets extracellular calcium for the scenario. | 1.2 | mM |
| `temperature` | Sets temperature for the scenario. | 295 | — |
...
```bash
biosimulant labs import labs/pasek2006-ventricular-cardiomyocytes
```
...
- License: `CC0` (from upstream BioModels entry [biomodels_ebi:MODEL0406553884](https://www.ebi.ac.uk/biomodels/MODEL0406553884)).
- This wrapper does not modify the upstream biology. To change rates, initial conditions, or kinetic laws, edit the SBML file in `models/core/data/MODEL0406553884.xml` directly.
...
| `Cmt` | Cmt | _(unset)_ |
| `Rst` | Rst | _(unset)_ |
| `VSRrel` | VSRrel | _(unset)_ |
| `VSRup` | VSRup | _(unset)_ |
| `Vc` | Vc | _(unset)_ |
| `pt` | pt | _(unset)_ |
| `Lt` | Lt | _(unset)_ |
| `Rot` | Rot | 83.33 |
| `radiust` | radiust | 1.5e-05 |
| `ptcm` | ptcm | 30000000 |
| `Smtot` | Smtot | 0.0001 |
| `i_circ` | i_circ | _(unset)_ |
| `i_Stim` | i_Stim | _(unset)_ |
| `stim_offset` | stim_offset | 0 |
| `stim_period` | stim_period | 1 |
| `stim_duration` | stim_duration | 0.001 |
| `stim_amplitude` | stim_amplitude | 0.0052 |
| `past` | past | _(unset)_ |
| `gNas` | gNas | _(unset)_ |
| `gCas` | gCas | _(unset)_ |
| `gKfs` | gKfs | _(unset)_ |
| `gKsts` | gKsts | _(unset)_ |
| `gK1s` | gK1s | _(unset)_ |
| `gKbs` | gKbs | _(unset)_ |
| `gKtos` | gKtos | _(unset)_ |
| `gNabs` | gNabs | _(unset)_ |
| `gCabs` | gCabs | _(unset)_ |
| `i_pCa_maxs` | i_pCa_maxs | _(unset)_ |
| `i_NaK_maxs` | i_NaK_maxs | _(unset)_ |
| `i_NaCa_maxs` | i_NaCa_maxs | _(unset)_ |
| `gNat` | gNat | _(unset)_ |
| `gCat` | gCat | _(unset)_ |
| `gKft` | gKft | _(unset)_ |
| `gKstt` | gKstt | _(unset)_ |
| `gK1t` | gK1t | _(unset)_ |
| `gKbt` | gKbt | _(unset)_ |
| `gKtot` | gKtot | _(unset)_ |
| `gNabt` | gNabt | _(unset)_ |
| _... 167 more, see SBML file_ | | |

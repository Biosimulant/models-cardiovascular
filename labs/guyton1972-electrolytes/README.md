# Guyton1972 Electrolytes Lab

This lab asks: **How does electrolyte balance shift?**
...
The lab opens as a canvas with three model nodes wired in series: the scenario driver on the left, the core simulator in the middle, and the visualization sub-model on the right. After running, the viz node emits four visualizations: an event-annotated absolute timeseries, a baseline-relative response timeseries, a signed response bar chart, and a question-and-answer table titled `What Happened` that answers `How does electrolyte balance shift?` in plain language. The viz node also publishes the table as a structured `run_summary` record that downstream nodes can consume.
...
1. `guyton1972_electrolytes_core`: SBML simulator. Receives every contextual input port (ion concentrations, voltages, conductances, etc. — see below), drives roadrunner, and emits two outputs: `state` (per-window species/state-variable record) and `species_labels` (a one-time map of `{species_id: human_label}` lifted from the SBML `<species name=...>` attributes).
2. `guyton1972_electrolytes_viz`: visualization sub-model. Consumes both `state` and `species_labels` from the core, accumulates the per-window history, prettifies the labels, and renders the two visuals plus the `run_summary` output. No SBML, no roadrunner — pure presentation.
...
- `models/core/src/guyton1972_electrolytes_model0912160001_model.py` is the SBML wrapper (no visualization code).
- `models/core/data/MODEL0912160001.xml` is the original SBML file from BioModels.
...
- `models/viz/src/guyton1972_electrolytes_viz.py` consumes core state + labels and renders timeseries, Q&A table, and the `run_summary` record.
...
|---|---|---|---|
| `sodium_intake` | Sets sodium intake for the scenario. | 0.1 | — |
| `potassium_intake` | Sets potassium intake for the scenario. | 0.08 | — |
| `aldosterone_drive` | Sets aldosterone drive for the scenario. | 1.037 | — |
...
```bash
biosimulant labs import labs/guyton1972-electrolytes
```
...
- License: `CC0` (from upstream BioModels entry [biomodels_ebi:MODEL0912160001](https://www.ebi.ac.uk/biomodels/MODEL0912160001)).
- This wrapper does not modify the upstream biology. To change rates, initial conditions, or kinetic laws, edit the SBML file in `models/core/data/MODEL0912160001.xml` directly.

...
"""Auto-generated SBML BioModule wrapper for Irvine1999_CardiacSodiumChannel.
...
class SbmlIrvine1999Cardiacsodiumchannel(biosim.BioModule):
    """BioModule wrapper for SBML model: Irvine1999_CardiacSodiumChannel."""
    ...
    def __init__(self, model_path: str = "data/MODEL0848062679.xml", integration_step: float = 0.01) -> None:
    ...
    def inputs(self) -> dict[str, SignalSpec]:
        return {
            "holding_voltage": SignalSpec.scalar(
                dtype="float64",
                accepted_profiles=(
                    AcceptedSignalProfile(signal_type="scalar", dtype="float64", accepted_units=("mV",)),
                    AcceptedSignalProfile(signal_type="record", schema={"payload": "json"}),
                ),
                description="Sets holding voltage for the simulation scenario. Default `-0.12`.",
            ),
            "sodium_reversal_voltage": SignalSpec.scalar(
                dtype="float64",
                accepted_profiles=(
                    AcceptedSignalProfile(signal_type="scalar", dtype="float64", accepted_units=("mV",)),
                    AcceptedSignalProfile(signal_type="record", schema={"payload": "json"}),
                ),
                description="Sets sodium reversal voltage for the simulation scenario. Default `0.044675`.",
            ),
            "sodium_conductance": SignalSpec.scalar(
                dtype="float64",
                accepted_profiles=(
                    AcceptedSignalProfile(signal_type="scalar", dtype="float64", accepted_units=("mS_per_uF",)),
                    AcceptedSignalProfile(signal_type="record", schema={"payload": "json"}),
                ),
                description="Sets sodium conductance for the simulation scenario. Default `0.0131`.",
            ),

...
This lab asks: **Does vascular stress relaxation buffer pressure?**
...
- Delayed stress-relaxation response
- Venous stress-relaxation state 1
- Venous stress-relaxation state 2

The captured run applies the configured effective volume challenge, changing effective volume from `0.743224` to `0.8547076` during the challenge window before returning to baseline. It evaluates 1 s of simulated dynamics across Stress-relaxation response, Delayed stress-relaxation response, Venous stress-relaxation state 1, Venous stress-relaxation state 2; read the absolute trajectories, baseline-relative plot, response ranking, and summary table together because the variables use different native scales.
...
1. `guyton1972_stress_relaxation_core`: SBML simulator. Receives every contextual input port (ion concentrations, voltages, conductances, etc. — see below), drives roadrunner, and emits two outputs: `state` (per-window species/state-variable record) and `species_labels` (a one-time map of `{species_id: human_label}` lifted from the SBML `<species name=...>` attributes).
2. `guyton1972_stress_relaxation_viz`: visualization sub-model. Consumes both `state` and `species_labels` from the core, accumulates the per-window history, prettifies the labels, and renders the two visuals plus the `run_summary` output. No SBML, no roadrunner — pure presentation.
...
- `models/core/src/guyton1972_stressrelaxation_model0910896131_model.py` is the SBML wrapper (no visualization code).
- `models/core/data/MODEL0910896131.xml` is the original SBML file from BioModels.
...
- `models/viz/src/guyton1972_stress_relaxation_viz.py` consumes core state + labels and renders timeseries, Q&A table, and the `run_summary` record.
...
|---|---|---|---|
| `effective_volume` | Sets effective volume for the scenario. | 0.743224 | — |
| `stress_relaxation_gain` | Sets stress relaxation gain for the scenario. | 5 | — |
| `stress_relaxation_gain_2` | Sets stress relaxation gain 2 for the scenario. | 10000 | — |
...
```bash
biosimulant labs import labs/guyton1972-stress-relaxation
```
...
- License: `CC0` (from upstream BioModels entry [biomodels_ebi:MODEL0910896131](https://www.ebi.ac.uk/biomodels/MODEL0910896131)).
- This wrapper does not modify the upstream biology. To change rates, initial conditions, or kinetic laws, edit the SBML file in `models/core/data/MODEL0910896131.xml` directly.

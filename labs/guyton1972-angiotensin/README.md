# Guyton1972 Angiotensin Lab

This lab asks: **How strongly is the angiotensin system activated?**
...
The lab opens as a canvas with three model nodes wired in series: the scenario driver on the left, the core simulator in the middle, and the visualization sub-model on the right. After running, the viz node emits four visualizations: an event-annotated absolute timeseries, a baseline-relative response timeseries, a signed response bar chart, and a question-and-answer table titled `What Happened` that answers `How strongly is the angiotensin system activated?` in plain language. The viz node also publishes the table as a structured `run_summary` record that downstream nodes can consume.
...
- Angiotensin tissue effect
- Angiotensin multiplier
- Angiotensin urine-flow effect
- Angiotensin vascular effect
- Angiotensin pressure response

The first screenshot shows the canvas and results panel with the absolute angiotensin-system trajectories and baseline-relative infusion response. The second scrolls down to the ranked response chart and the `What Happened` Q&A table for the same run.

![Guyton1972 angiotensin lab canvas with absolute and baseline-relative infusion response trajectories](assets/guyton1972-angiotensin-timeseries-results.png)

![Guyton1972 angiotensin ranked response chart and What Happened summary table](assets/guyton1972-angiotensin-response-summary.png)

## How the Models Connect

The canvas has two steps:

1. `guyton1972_angiotensin_core`: SBML simulator. Receives every contextual input port (ion concentrations, voltages, conductances, etc. — see below), drives roadrunner, and emits two outputs: `state` (per-window species/state-variable record) and `species_labels` (a one-time map of `{species_id: human_label}` lifted from the SBML `<species name=...>` attributes).
2. `guyton1972_angiotensin_viz`: visualization sub-model. Consumes both `state` and `species_labels` from the core, accumulates the per-window history, prettifies the labels, and renders the two visuals plus the `run_summary` output. No SBML, no roadrunner — pure presentation.

## How to Read the Visualizations

The absolute timeseries plot has one curve per state variable in the SBML model and marks when the challenge and recovery windows begin. The baseline-relative plot rescales each curve against the pre-challenge baseline so small but real responses are visible. The x-axis is simulation time in seconds and the y-axis is the variable's value in its native SBML unit (concentrations in mM or mol, voltages in mV, currents in pA, volumes in mL — refer to the SBML file for the exact unit per variable). In the default screenshot, the angiotensin infusion challenge starts at 150 s, changes infusion from 0 to 1, and recovery starts at 450 s.

The response bar chart ranks final-minus-baseline changes in native SBML units. The "What Happened" table explains the lab question, the applied challenge, the simulated duration, the strongest response, and the interpretation limits. In the shown run, angiotensin concentration is the largest final-minus-baseline responder, increasing by about 0.00877 native SBML units.
...
- `models/core/src/guyton1972_angiotensin_model0911342562_model.py` is the SBML wrapper (no visualization code).
- `models/core/data/MODEL0911342562.xml` is the original SBML file from BioModels.
...
- `models/viz/src/guyton1972_angiotensin_viz.py` consumes core state + labels and renders timeseries, Q&A table, and the `run_summary` record.
...
|---|---|---|---|
| `renal_perfusion` | Sets renal perfusion for the scenario. | 1.00051 | — |
| `sympathetic_drive` | Sets sympathetic drive for the scenario. | 0.4 | — |
| `angiotensin_infusion` | Sets angiotensin infusion for the scenario. | 0 | — |
...
```bash
biosimulant labs import labs/guyton1972-angiotensin
```
...
- License: `CC0` (from upstream BioModels entry [biomodels_ebi:MODEL0911342562](https://www.ebi.ac.uk/biomodels/MODEL0911342562)).
- This wrapper does not modify the upstream biology. To change rates, initial conditions, or kinetic laws, edit the SBML file in `models/core/data/MODEL0911342562.xml` directly.
...
|---|---|---|
| `MDFLW` | MDFLW | 1.00051 |
| `ANGSCR` | ANGSCR | _(unset)_ |
| `MDFLW3` | MDFLW3 | _(unset)_ |
| `ANX1` | ANX1 | 0 |
| `ANX` | ANX | _(unset)_ |
| `ANPR` | ANPR | _(unset)_ |
| `ANPRT` | ANPRT | _(unset)_ |
| `ANPR1` | ANPR1 | _(unset)_ |
| `ANC` | ANC | 0.859476 |
| `ANM` | ANM | _(unset)_ |
| `ANU` | ANU | _(unset)_ |
| `ANU1` | ANU1 | _(unset)_ |
| `ANUVN` | ANUVN | _(unset)_ |
| `ANXM` | ANXM | 0 |
| `ANV` | ANV | 5000 |
| `REK` | REK | 1 |
| `ANGINF` | ANGINF | 0 |

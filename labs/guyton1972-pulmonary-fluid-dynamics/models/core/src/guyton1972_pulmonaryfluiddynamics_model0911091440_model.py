...
"""Auto-generated SBML BioModule wrapper for Guyton1972_PulmonaryFluidDynamics.
...
class SbmlGuyton1972Pulmonaryfluiddynamics(biosim.BioModule):
    """BioModule wrapper for SBML model: Guyton1972_PulmonaryFluidDynamics."""

    _PORT_TO_SBML: dict[str, str] = {
        "capillary_pressure": "PPC",
    ...
    def __init__(self, model_path: str = "data/MODEL0911091440.xml", integration_step: float = 0.01) -> None:
    ...
    def inputs(self) -> dict[str, SignalSpec]:
        return {
            "capillary_pressure": SignalSpec.scalar(
                dtype="float64",
                accepted_profiles=(
                    AcceptedSignalProfile(signal_type="scalar", dtype="float64", accepted_units=("dimensionless",)),
                    AcceptedSignalProfile(signal_type="record", schema={"payload": "json"}),
                ),
                description="Sets capillary pressure for the simulation scenario. Default `29.9941`.",
            ),
            "plasma_oncotic_pressure": SignalSpec.scalar(
                dtype="float64",
                accepted_profiles=(
                    AcceptedSignalProfile(signal_type="scalar", dtype="float64", accepted_units=("dimensionless",)),
                    AcceptedSignalProfile(signal_type="record", schema={"payload": "json"}),
                ),
                description="Sets plasma oncotic pressure for the simulation scenario. Default `71.9719`.",
            ),

...
"""Auto-generated SBML BioModule wrapper for Karlstaedt2019 - G6P accumulation via Phosphoglucose isomerase inhibition in heart muscles.
...
class SbmlKarlstaedt2019G6pAccumulationViaPhosphoglucose(biosim.BioModule):
    """BioModule wrapper for SBML model: Karlstaedt2019 - G6P accumulation via Phosphoglucose isomerase inhibition in heart muscles."""

    _PORT_TO_SBML: dict[str, str] = {
        "extracellular_glucose": "GLCo",
    ...
    def __init__(self, model_path: str = "data/MODEL1910170001.xml", integration_step: float = 0.01) -> None:
    ...
    def inputs(self) -> dict[str, SignalSpec]:
        return {
            "extracellular_glucose": SignalSpec.scalar(
                dtype="float64",
                accepted_profiles=(
                    AcceptedSignalProfile(signal_type="scalar", dtype="float64", accepted_units=("dimensionless",)),
                    AcceptedSignalProfile(signal_type="record", schema={"payload": "json"}),
                ),
                description="Sets extracellular glucose for the simulation scenario. Default `5.5`.",
            ),
            "cellular_energy_pool": SignalSpec.scalar(
                dtype="float64",
                accepted_profiles=(
                    AcceptedSignalProfile(signal_type="scalar", dtype="float64", accepted_units=("dimensionless",)),
                    AcceptedSignalProfile(signal_type="record", schema={"payload": "json"}),
                ),
                description="Sets cellular energy pool for the simulation scenario. Default `3.0417`.",
            ),

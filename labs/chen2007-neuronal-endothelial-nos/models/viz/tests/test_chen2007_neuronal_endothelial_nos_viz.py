...
"""Tests for the Chen2007 Neuronal Endothelial Nos visualization sub-model. No SBML or
...
from src.chen2007_neuronal_endothelial_nos_viz import Chen2007NeuronalendothelialnosModel0491251823ModelViz  # noqa: E402
...
def test_instantiation():
    m = Chen2007NeuronalendothelialnosModel0491251823ModelViz()
...
def test_lab_question_threads_through_visuals():
    m = Chen2007NeuronalendothelialnosModel0491251823ModelViz(lab_question="Does this experiment work?")
    state = _record("state", {"x": 1.0})
    m.advance_window(0.0, 0.5, inputs={"state": state})
    visuals = m.visualize()
...
def test_generic_title_when_lab_question_blank():
    m = Chen2007NeuronalendothelialnosModel0491251823ModelViz(lab_question="")
    state = _record("state", {"x": 1.0})
    m.advance_window(0.0, 0.5, inputs={"state": state})
    visuals = m.visualize()
    assert visuals[0]["data"]["title"] == "What did the model simulate?"

...
"""Tests for the Clancy2001 Potassium Channel visualization sub-model. No SBML or
...
from src.clancy2001_potassium_channel_viz import Clancy2001KchannelBiomd0000000121ModelViz  # noqa: E402
...
def test_instantiation():
    m = Clancy2001KchannelBiomd0000000121ModelViz()
...
def test_lab_question_threads_through_visuals():
    m = Clancy2001KchannelBiomd0000000121ModelViz(lab_question="Does this experiment work?")
    state = _record("state", {"x": 1.0})
    m.advance_window(0.0, 0.5, inputs={"state": state})
    visuals = m.visualize()
...
def test_generic_title_when_lab_question_blank():
    m = Clancy2001KchannelBiomd0000000121ModelViz(lab_question="")
    state = _record("state", {"x": 1.0})
    m.advance_window(0.0, 0.5, inputs={"state": state})
    visuals = m.visualize()
    assert visuals[0]["data"]["title"] == "What did the model simulate?"

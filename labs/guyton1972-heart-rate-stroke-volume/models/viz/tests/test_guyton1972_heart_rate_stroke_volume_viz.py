...
"""Tests for the Guyton1972 Heart Rate Stroke Volume visualization sub-model. No SBML or
...
from src.guyton1972_heart_rate_stroke_volume_viz import Guyton1972HeartratestrokevolumeModel0911270006ModelViz  # noqa: E402
...
def test_instantiation():
    m = Guyton1972HeartratestrokevolumeModel0911270006ModelViz()
...
def test_lab_question_threads_through_visuals():
    m = Guyton1972HeartratestrokevolumeModel0911270006ModelViz(lab_question="Does this experiment work?")
    state = _record("state", {"x": 1.0})
    m.advance_window(0.0, 0.5, inputs={"state": state})
    visuals = m.visualize()
...
def test_generic_title_when_lab_question_blank():
    m = Guyton1972HeartratestrokevolumeModel0911270006ModelViz(lab_question="")
    state = _record("state", {"x": 1.0})
    m.advance_window(0.0, 0.5, inputs={"state": state})
    visuals = m.visualize()
    assert visuals[0]["data"]["title"] == "What did the model simulate?"

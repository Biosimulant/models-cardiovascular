# Guyton1972 Red Cells Viscosity Lab

This lab asks: **How do red cells change blood viscosity?**
...
The lab opens as a canvas with three model nodes wired in series: the scenario driver on the left, the core simulator in the middle, and the visualization sub-model on the right. After running, the viz node emits four visualizations: an event-annotated absolute timeseries, a baseline-relative response timeseries, a signed response bar chart, and a question-and-answer table titled `What Happened` that answers `How do red cells change blood viscosity?` in plain language. The viz node also publishes the table as a structured `run_summary` record that downstream nodes can consume.
...
- Blood volume
- Hematocrit
- Blood viscosity multiplier

The captured run applies the configured cardiovascular control challenge, changing erythropoiesis drive from `150` to `225` during the challenge window before returning to baseline. It evaluates 600 s of simulated dynamics across Red-cell volume, Blood volume, Hematocrit, Blood viscosity multiplier; read the absolute trajectories, baseline-relative plot, response ranking, and summary table together because the variables use different native scales.
...
1. `guyton1972_red_cells_viscosity_core`: SBML simulator. Receives every contextual input port (ion concentrations, voltages, conductances, etc. — see below), drives roadrunner, and emits two outputs: `state` (per-window species/state-variable record) and `species_labels` (a one-time map of `{species_id: human_label}` lifted from the SBML `<species name=...>` attributes).
2. `guyton1972_red_cells_viscosity_viz`: visualization sub-model. Consumes both `state` and `species_labels` from the core, accumulates the per-window history, prettifies the labels, and renders the two visuals plus the `run_summary` output. No SBML, no roadrunner — pure presentation.
...
- `models/core/src/guyton1972_redcells_viscosity_model0910928451_model.py` is the SBML wrapper (no visualization code).
- `models/core/data/MODEL0910928451.xml` is the original SBML file from BioModels.
...
- `models/viz/src/guyton1972_red_cells_viscosity_viz.py` consumes core state + labels and renders timeseries, Q&A table, and the `run_summary` record.
...
|---|---|---|---|
| `plasma_volume` | Sets plasma volume for the scenario. | 3.00449 | — |
| `erythropoiesis_drive` | Sets erythropoiesis drive for the scenario. | 150 | — |
...
```bash
biosimulant labs import labs/guyton1972-red-cells-viscosity
```
...
- License: `CC0` (from upstream BioModels entry [biomodels_ebi:MODEL0910928451](https://www.ebi.ac.uk/biomodels/MODEL0910928451)).
- This wrapper does not modify the upstream biology. To change rates, initial conditions, or kinetic laws, edit the SBML file in `models/core/data/MODEL0910928451.xml` directly.

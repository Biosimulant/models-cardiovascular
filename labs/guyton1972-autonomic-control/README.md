# Guyton1972 Autonomic Control Lab

This lab asks: **How does autonomic feedback stabilize circulation?**
...
The lab opens as a canvas with three model nodes wired in series: the scenario driver on the left, the core simulator in the middle, and the visualization sub-model on the right. After running, the viz node emits four visualizations: an event-annotated absolute timeseries, a baseline-relative response timeseries, a signed response bar chart, and a question-and-answer table titled `What Happened` that answers `How does autonomic feedback stabilize circulation?` in plain language. The viz node also publishes the table as a structured `run_summary` record that downstream nodes can consume.
...
- Autonomic response
- Autonomic control signal
- Delayed autonomic response
- Secondary autonomic response

The first screenshot shows the canvas and results panel with the absolute autonomic-control trajectories and baseline-relative arterial-oxygen challenge response. The second scrolls down to the ranked response chart and the `What Happened` Q&A table for the same run.

![Guyton1972 autonomic control lab canvas with absolute and baseline-relative oxygen challenge trajectories](assets/guyton1972-autonomic-timeseries-results.png)

![Guyton1972 autonomic control ranked response chart and What Happened summary table](assets/guyton1972-autonomic-response-summary.png)

## How the Models Connect

The canvas has two steps:

1. `guyton1972_autonomic_control_core`: SBML simulator. Receives every contextual input port (ion concentrations, voltages, conductances, etc. — see below), drives roadrunner, and emits two outputs: `state` (per-window species/state-variable record) and `species_labels` (a one-time map of `{species_id: human_label}` lifted from the SBML `<species name=...>` attributes).
2. `guyton1972_autonomic_control_viz`: visualization sub-model. Consumes both `state` and `species_labels` from the core, accumulates the per-window history, prettifies the labels, and renders the two visuals plus the `run_summary` output. No SBML, no roadrunner — pure presentation.

## How to Read the Visualizations

The absolute timeseries plot has one curve per state variable in the SBML model and marks when the challenge and recovery windows begin. The baseline-relative plot rescales each curve against the pre-challenge baseline so small but real responses are visible. The x-axis is simulation time in seconds and the y-axis is the variable's value in its native SBML unit (concentrations in mM or mol, voltages in mV, currents in pA, volumes in mL — refer to the SBML file for the exact unit per variable). In the default screenshot, the arterial-oxygen challenge starts at 150 s, reduces arterial oxygen from 97.0439 to 48.52195, and recovery starts at 450 s.

The response bar chart ranks final-minus-baseline changes in native SBML units. The "What Happened" table explains the lab question, the applied challenge, the simulated duration, the strongest response, and the interpretation limits. In the shown run, autonomic control signal is the largest final-minus-baseline responder, increasing by about 0.0115 native SBML units.
...
- `models/core/src/guyton1972_autonomics_model0911270005_model.py` is the SBML wrapper (no visualization code).
- `models/core/data/MODEL0911270005.xml` is the original SBML file from BioModels.
...
- `models/viz/src/guyton1972_autonomic_control_viz.py` consumes core state + labels and renders timeseries, Q&A table, and the `run_summary` record.
...
| `arterial_oxygen` | Sets arterial oxygen for the scenario. | 97.0439 | — |
...
```bash
biosimulant labs import labs/guyton1972-autonomic-control
```
...
- License: `CC0` (from upstream BioModels entry [biomodels_ebi:MODEL0911270005](https://www.ebi.ac.uk/biomodels/MODEL0911270005)).
- This wrapper does not modify the upstream biology. To change rates, initial conditions, or kinetic laws, edit the SBML file in `models/core/data/MODEL0911270005.xml` directly.
...
| `AULP` | AULP | _(unset)_ |
| `AUEX` | AUEX | _(unset)_ |
| `AUTTL` | AUTTL | _(unset)_ |
| `AUTTL1` | AUTTL1 | _(unset)_ |
| `AU` | AU | _(unset)_ |
| `DAU` | DAU | _(unset)_ |
| `AU1` | AU1 | 1.00007 |
| `AUT` | AUT | _(unset)_ |

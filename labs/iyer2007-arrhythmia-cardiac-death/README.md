# Iyer2007 Arrhythmia Cardiac Death Lab

This lab asks: **Does abnormal calcium handling create arrhythmogenic behaviour?**

The lab is composed of two sub-models: a `core` SBML simulator (encoded as SBML, solved by Tellurium) and a dedicated `viz` presenter that turns the raw simulation state into a friendly timeseries plot and a plain-language "What Happened" summary. The split keeps the SBML wrapper clean and makes it easy to swap or extend the visualization without touching the simulator. This a model from the article: Mechanisms of abnormal calcium homeostasis in mutations responsible forcatecholaminergic polymorphic ventricular tachycardia. It can be used to explore cardiac dynamics and compare response patterns across conditions.

## What You'll See

The lab opens as a canvas with three model nodes wired in series: the scenario driver on the left, the core simulator in the middle, and the visualization sub-model on the right. After running, the viz node emits four visualizations: an event-annotated absolute timeseries, a baseline-relative response timeseries, a signed response bar chart, and a question-and-answer table titled `What Happened` that answers `Does abnormal calcium handling create arrhythmogenic behaviour?` in plain language. The viz node also publishes the table as a structured `run_summary` record that downstream nodes can consume.
...
The captured run applies the configured calcium activation challenge, changing intracellular calcium from `0.00036396867218` to `0.00072793734436` during the challenge window before returning to baseline. It evaluates 1 s of simulated dynamics across Membrane voltage, Cytosolic calcium, Subspace calcium, Junctional sarcoplasmic-reticulum calcium, and 4 other tracked variables; read the absolute trajectories, baseline-relative plot, response ranking, and summary table together because the variables use different native scales.
...
1. `iyer2007_arrhythmia_cardiac_death_core`: SBML simulator. Receives every contextual input port (ion concentrations, voltages, conductances, etc. — see below), drives roadrunner, and emits two outputs: `state` (per-window species/state-variable record) and `species_labels` (a one-time map of `{species_id: human_label}` lifted from the SBML `<species name=...>` attributes).
2. `iyer2007_arrhythmia_cardiac_death_viz`: visualization sub-model. Consumes both `state` and `species_labels` from the core, accumulates the per-window history, prettifies the labels, and renders the two visuals plus the `run_summary` output. No SBML, no roadrunner — pure presentation.
...
- `models/core/src/iyer2007_arrhythmia_cardiacdeath_model1006230030_model.py` is the SBML wrapper (no visualization code).
- `models/core/data/MODEL1006230030.xml` is the original SBML file from BioModels.
...
- `models/viz/src/iyer2007_arrhythmia_cardiac_death_viz.py` consumes core state + labels and renders timeseries, Q&A table, and the `run_summary` record.
...
| `isoproterenol_drive` | Sets isoproterenol drive for the scenario. | 0 | — |
| `calsequestrin_buffer` | Sets calsequestrin buffer for the scenario. | 0 | — |
| `ryanodine_receptor_pool` | Sets ryanodine receptor pool for the scenario. | 0 | — |
| `intracellular_calcium` | Sets intracellular calcium for the scenario. | 0.00036396867218 | mM |
...
```bash
biosimulant labs import labs/iyer2007-arrhythmia-cardiac-death
```
...
- License: `CC0` (from upstream BioModels entry [biomodels_ebi:MODEL1006230030](https://www.ebi.ac.uk/biomodels/MODEL1006230030)).
- This wrapper does not modify the upstream biology. To change rates, initial conditions, or kinetic laws, edit the SBML file in `models/core/data/MODEL1006230030.xml` directly.
...
| `VNSR` | VNSR | 2.1e-06 |
| `VSS` | VSS | 1.2e-09 |
| `Nai` | Nai | 9.85573275838928 |
| `Ki` | Ki | 125.427082712469 |
| `Cai` | Cai | 0.00036396867218 |
| `CaSS` | CaSS | 0.00050660427804 |
| `CaJSR` | CaJSR | 0.42193698051504 |
| `CaNSR` | CaNSR | 0.42355162144024 |
| `V` | V | -86.7261544519706 |
| `i_tot` | i_tot | _(unset)_ |
| `Ko` | Ko | 4 |
| `Nao` | Nao | 138 |
| `Cao` | Cao | 2 |
| `i_Stim` | i_Stim | _(unset)_ |
| `stim_period` | stim_period | 500 |
| `stim_duration` | stim_duration | 3 |
| `stim_amplitude` | stim_amplitude | -15 |
| `stim_offset` | stim_offset | 100 |
| `past` | past | _(unset)_ |
| `fb` | fb | _(unset)_ |
| `Kfb` | Kfb | 0.000168 |
| `Nfb` | Nfb | 1.2 |

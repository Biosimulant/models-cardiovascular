# Guyton1972 Pulmonary Oxygen Intake Lab

This lab asks: **Is pulmonary oxygen uptake sufficient?**
...
The lab opens as a canvas with three model nodes wired in series: the scenario driver on the left, the core simulator in the middle, and the visualization sub-model on the right. After running, the viz node emits four visualizations: an event-annotated absolute timeseries, a baseline-relative response timeseries, a signed response bar chart, and a question-and-answer table titled `What Happened` that answers `Is pulmonary oxygen uptake sufficient?` in plain language. The viz node also publishes the table as a structured `run_summary` record that downstream nodes can consume.
...
The captured run applies the configured oxygen availability challenge, changing oxygen demand from `56.8057` to `28.40285` during the challenge window before returning to baseline. It evaluates 600 s of simulated dynamics across Alveolar oxygen pressure, Arterial oxygen pressure, Arterial oxygen saturation, Venous oxygen saturation; read the absolute trajectories, baseline-relative plot, response ranking, and summary table together because the variables use different native scales.
...
1. `guyton1972_pulmonary_oxygen_intake_core`: SBML simulator. Receives every contextual input port (ion concentrations, voltages, conductances, etc. — see below), drives roadrunner, and emits two outputs: `state` (per-window species/state-variable record) and `species_labels` (a one-time map of `{species_id: human_label}` lifted from the SBML `<species name=...>` attributes).
2. `guyton1972_pulmonary_oxygen_intake_viz`: visualization sub-model. Consumes both `state` and `species_labels` from the core, accumulates the per-window history, prettifies the labels, and renders the two visuals plus the `run_summary` output. No SBML, no roadrunner — pure presentation.
...
- `models/core/src/guyton1972_pulmonaryoxygenintake_model0911047946_model.py` is the SBML wrapper (no visualization code).
- `models/core/data/MODEL0911047946.xml` is the original SBML file from BioModels.
...
- `models/viz/src/guyton1972_pulmonary_oxygen_intake_viz.py` consumes core state + labels and renders timeseries, Q&A table, and the `run_summary` record.
...
|---|---|---|---|
| `pulmonary_blood_flow` | Sets pulmonary blood flow for the scenario. | 4.97838 | — |
| `oxygen_demand` | Sets oxygen demand for the scenario. | 56.8057 | — |
| `ambient_oxygen` | Sets ambient oxygen for the scenario. | 150 | — |
...
```bash
biosimulant labs import labs/guyton1972-pulmonary-oxygen-intake
```
...
- License: `CC0` (from upstream BioModels entry [biomodels_ebi:MODEL0911047946](https://www.ebi.ac.uk/biomodels/MODEL0911047946)).
- This wrapper does not modify the upstream biology. To change rates, initial conditions, or kinetic laws, edit the SBML file in `models/core/data/MODEL0911047946.xml` directly.
...
| `RMO` | RMO | 56.8057 |
| `HM` | HM | 40.0381 |
| `O2UTIL` | O2UTIL | _(unset)_ |
| `ALVENT` | ALVENT | _(unset)_ |
| `PO2ALV` | PO2ALV | _(unset)_ |
| `O2DFS` | O2DFS | _(unset)_ |
| `RSPDFC` | RSPDFC | _(unset)_ |
| `OVA` | OVA | 204.497 |
| `DOVA` | DOVA | _(unset)_ |
| `PO2ART` | PO2ART | _(unset)_ |
| `OSA` | OSA | _(unset)_ |
| `O2VTS2` | O2VTS2 | _(unset)_ |
| `O2VTST` | O2VTST | _(unset)_ |
| `O2VTST1` | O2VTST1 | _(unset)_ |
| `O2VAD2` | O2VAD2 | _(unset)_ |
| `DO2VAD` | DO2VAD | _(unset)_ |
| `O2VAD1` | O2VAD1 | 2.368e-07 |
| `PO2AMB` | PO2AMB | 150 |
| `PL2` | PL2 | 1.8 |
| `VPTISS` | VPTISS | 0.0175 |
| `VNTSTM` | VNTSTM | 1 |

...
"""Auto-generated SBML BioModule wrapper for Guyton1972_Angiotensin.
...
class SbmlGuyton1972Angiotensin(biosim.BioModule):
    """BioModule wrapper for SBML model: Guyton1972_Angiotensin."""

    _PORT_TO_SBML: dict[str, str] = {
        "renal_perfusion": "MDFLW",
    ...
    def __init__(self, model_path: str = "data/MODEL0911342562.xml", integration_step: float = 0.01) -> None:
    ...
    def inputs(self) -> dict[str, SignalSpec]:
        return {
            "renal_perfusion": SignalSpec.scalar(
                dtype="float64",
                accepted_profiles=(
                    AcceptedSignalProfile(signal_type="scalar", dtype="float64", accepted_units=("dimensionless",)),
                    AcceptedSignalProfile(signal_type="record", schema={"payload": "json"}),
                ),
                description="Sets renal perfusion for the simulation scenario. Default `1.00051`.",
            ),
            "sympathetic_drive": SignalSpec.scalar(
                dtype="float64",
                accepted_profiles=(
                    AcceptedSignalProfile(signal_type="scalar", dtype="float64", accepted_units=("dimensionless",)),
                    AcceptedSignalProfile(signal_type="record", schema={"payload": "json"}),
                ),
                description="Sets sympathetic drive for the simulation scenario. Default `0.4`.",
            ),
            "angiotensin_infusion": SignalSpec.scalar(
                dtype="float64",
                accepted_profiles=(
                    AcceptedSignalProfile(signal_type="scalar", dtype="float64", accepted_units=("dimensionless",)),
                    AcceptedSignalProfile(signal_type="record", schema={"payload": "json"}),
                ),
                description="Sets angiotensin infusion for the simulation scenario. Default `0.0`.",
            ),

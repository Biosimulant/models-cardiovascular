# Guyton1972 Heart Rate Stroke Volume Lab

This lab asks: **How do heart rate and stroke volume respond?**
...
The lab opens as a canvas with three model nodes wired in series: the scenario driver on the left, the core simulator in the middle, and the visualization sub-model on the right. After running, the viz node emits four visualizations: an event-annotated absolute timeseries, a baseline-relative response timeseries, a signed response bar chart, and a question-and-answer table titled `What Happened` that answers `How do heart rate and stroke volume respond?` in plain language. The viz node also publishes the table as a structured `run_summary` record that downstream nodes can consume.
...
- Autonomic heart-rate drive
- Pressure-reflex heart-rate drive
- Hemodynamic heart-rate drive

The captured run applies the configured pressure-load challenge, changing right atrial pressure from `0.00852183` to `0.010226196` during the challenge window before returning to baseline. It evaluates 10 s of simulated dynamics across Heart rate, Stroke volume output, Autonomic heart-rate drive, Pressure-reflex heart-rate drive, and 1 other tracked variable; read the absolute trajectories, baseline-relative plot, response ranking, and summary table together because the variables use different native scales.
...
1. `guyton1972_heart_rate_stroke_volume_core`: SBML simulator. Receives every contextual input port (ion concentrations, voltages, conductances, etc. — see below), drives roadrunner, and emits two outputs: `state` (per-window species/state-variable record) and `species_labels` (a one-time map of `{species_id: human_label}` lifted from the SBML `<species name=...>` attributes).
2. `guyton1972_heart_rate_stroke_volume_viz`: visualization sub-model. Consumes both `state` and `species_labels` from the core, accumulates the per-window history, prettifies the labels, and renders the two visuals plus the `run_summary` output. No SBML, no roadrunner — pure presentation.
...
- `models/core/src/guyton1972_heartratestrokevolume_model0911270006_model.py` is the SBML wrapper (no visualization code).
- `models/core/data/MODEL0911270006.xml` is the original SBML file from BioModels.
...
- `models/viz/src/guyton1972_heart_rate_stroke_volume_viz.py` consumes core state + labels and renders timeseries, Q&A table, and the `run_summary` record.
...
|---|---|---|---|
| `autonomic_drive` | Sets autonomic drive for the scenario. | 1.3 | — |
| `right_atrial_pressure` | Sets right atrial pressure for the scenario. | 0.00852183 | — |
| `cardiac_output` | Sets cardiac output for the scenario. | 4.9943 | — |
...
```bash
biosimulant labs import labs/guyton1972-heart-rate-stroke-volume
```
...
- Default run length is `10` s with a `0.01` s communication step. These are conservative defaults chosen by category (single-cell electrophysiology vs whole-system circulation vs slow regulatory loop). Tune them in `lab.yaml`.
- Requires `tellurium==2.2.11.2`. The first import compiles the SBML to LLVM in-process.
- License: `CC0` (from upstream BioModels entry [biomodels_ebi:MODEL0911270006](https://www.ebi.ac.uk/biomodels/MODEL0911270006)).
- This wrapper does not modify the upstream biology. To change rates, initial conditions, or kinetic laws, edit the SBML file in `models/core/data/MODEL0911270006.xml` directly.

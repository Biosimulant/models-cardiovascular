...
"""Tests for the Iyer2004 Ventricular Myocyte visualization sub-model. No SBML or
...
from src.iyer2004_ventricular_myocyte_viz import Iyer2004VentricularmyocyteModel0847999575ModelViz  # noqa: E402
...
def test_instantiation():
    m = Iyer2004VentricularmyocyteModel0847999575ModelViz()
...
def test_lab_question_threads_through_visuals():
    m = Iyer2004VentricularmyocyteModel0847999575ModelViz(lab_question="Does this experiment work?")
    state = _record("state", {"x": 1.0})
    m.advance_window(0.0, 0.5, inputs={"state": state})
    visuals = m.visualize()
...
def test_generic_title_when_lab_question_blank():
    m = Iyer2004VentricularmyocyteModel0847999575ModelViz(lab_question="")
    state = _record("state", {"x": 1.0})
    m.advance_window(0.0, 0.5, inputs={"state": state})
    visuals = m.visualize()
    assert visuals[0]["data"]["title"] == "What did the model simulate?"

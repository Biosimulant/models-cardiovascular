...
"""Auto-generated SBML BioModule wrapper for Guyton1972_AntidiureticHormone.
...
class SbmlGuyton1972Antidiuretichormone(biosim.BioModule):
    """BioModule wrapper for SBML model: Guyton1972_AntidiureticHormone."""

    _PORT_TO_SBML: dict[str, str] = {
        "sodium_concentration": "CNA",
    ...
    def __init__(self, model_path: str = "data/MODEL0911309080.xml", integration_step: float = 0.01) -> None:
    ...
    def inputs(self) -> dict[str, SignalSpec]:
        return {
            "sodium_concentration": SignalSpec.scalar(
                dtype="float64",
                accepted_profiles=(
                    AcceptedSignalProfile(signal_type="scalar", dtype="float64", accepted_units=("dimensionless",)),
                    AcceptedSignalProfile(signal_type="record", schema={"payload": "json"}),
                ),
                description="Sets sodium concentration for the simulation scenario. Default `142.035`.",
            ),
            ...
            "antidiuretic_hormone_infusion": SignalSpec.scalar(
                dtype="float64",
                accepted_profiles=(
                    AcceptedSignalProfile(signal_type="scalar", dtype="float64", accepted_units=("dimensionless",)),
                    AcceptedSignalProfile(signal_type="record", schema={"payload": "json"}),
                ),
                description="Sets antidiuretic hormone infusion for the simulation scenario. Default `0.0`.",
            ),

...
"""Tests for the Irvine1999 Cardiac Sodium Channel visualization sub-model. No SBML or
...
from src.irvine1999_cardiac_sodium_channel_viz import Irvine1999CardiacsodiumchannelModel0848062679ModelViz  # noqa: E402
...
def test_instantiation():
    m = Irvine1999CardiacsodiumchannelModel0848062679ModelViz()
...
def test_lab_question_threads_through_visuals():
    m = Irvine1999CardiacsodiumchannelModel0848062679ModelViz(lab_question="Does this experiment work?")
    state = _record("state", {"x": 1.0})
    m.advance_window(0.0, 0.5, inputs={"state": state})
    visuals = m.visualize()
...
def test_generic_title_when_lab_question_blank():
    m = Irvine1999CardiacsodiumchannelModel0848062679ModelViz(lab_question="")
    state = _record("state", {"x": 1.0})
    m.advance_window(0.0, 0.5, inputs={"state": state})
    visuals = m.visualize()
    assert visuals[0]["data"]["title"] == "What did the model simulate?"

...
"""Auto-generated SBML BioModule wrapper for Guyton1972_StressRelaxation.
...
class SbmlGuyton1972Stressrelaxation(biosim.BioModule):
    """BioModule wrapper for SBML model: Guyton1972_StressRelaxation."""

    _PORT_TO_SBML: dict[str, str] = {
        "effective_volume": "VVE",
    ...
    def __init__(self, model_path: str = "data/MODEL0910896131.xml", integration_step: float = 0.01) -> None:
    ...
    def inputs(self) -> dict[str, SignalSpec]:
        return {
            "effective_volume": SignalSpec.scalar(
                dtype="float64",
                accepted_profiles=(
                    AcceptedSignalProfile(signal_type="scalar", dtype="float64", accepted_units=("dimensionless",)),
                    AcceptedSignalProfile(signal_type="record", schema={"payload": "json"}),
                ),
                description="Sets effective volume for the simulation scenario. Default `0.743224`.",
            ),
            "stress_relaxation_gain": SignalSpec.scalar(
                dtype="float64",
                accepted_profiles=(
                    AcceptedSignalProfile(signal_type="scalar", dtype="float64", accepted_units=("dimensionless",)),
                    AcceptedSignalProfile(signal_type="record", schema={"payload": "json"}),
                ),
                description="Sets stress relaxation gain for the simulation scenario. Default `5.0`.",
            ),
            "stress_relaxation_gain_2": SignalSpec.scalar(
                dtype="float64",
                accepted_profiles=(
                    AcceptedSignalProfile(signal_type="scalar", dtype="float64", accepted_units=("dimensionless",)),
                    AcceptedSignalProfile(signal_type="record", schema={"payload": "json"}),
                ),
                description="Sets stress relaxation gain 2 for the simulation scenario. Default `10000.0`.",
            ),

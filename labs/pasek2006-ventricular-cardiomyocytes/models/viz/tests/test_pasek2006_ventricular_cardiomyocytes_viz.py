...
"""Tests for the Pasek2006 Ventricular Cardiomyocytes visualization sub-model. No SBML or
...
from src.pasek2006_ventricular_cardiomyocytes_viz import Pasek2006VentricularcardiomyocytesModel0406553884ModelViz  # noqa: E402
...
def test_instantiation():
    m = Pasek2006VentricularcardiomyocytesModel0406553884ModelViz()
...
def test_lab_question_threads_through_visuals():
    m = Pasek2006VentricularcardiomyocytesModel0406553884ModelViz(lab_question="Does this experiment work?")
    state = _record("state", {"x": 1.0})
    m.advance_window(0.0, 0.5, inputs={"state": state})
    visuals = m.visualize()
...
def test_generic_title_when_lab_question_blank():
    m = Pasek2006VentricularcardiomyocytesModel0406553884ModelViz(lab_question="")
    state = _record("state", {"x": 1.0})
    m.advance_window(0.0, 0.5, inputs={"state": state})
    visuals = m.visualize()
    assert visuals[0]["data"]["title"] == "What did the model simulate?"

...
"""Tests for the Chen2006 Endothelial No Release visualization sub-model. No SBML or
...
from src.chen2006_endothelial_no_release_viz import Chen2006NitricOxideReleaseFromEndothelialCBiomd0000000676ModelViz  # noqa: E402
...
def test_instantiation():
    m = Chen2006NitricOxideReleaseFromEndothelialCBiomd0000000676ModelViz()
...
def test_lab_question_threads_through_visuals():
    m = Chen2006NitricOxideReleaseFromEndothelialCBiomd0000000676ModelViz(lab_question="Does this experiment work?")
    state = _record("state", {"x": 1.0})
    m.advance_window(0.0, 0.5, inputs={"state": state})
    visuals = m.visualize()
...
def test_generic_title_when_lab_question_blank():
    m = Chen2006NitricOxideReleaseFromEndothelialCBiomd0000000676ModelViz(lab_question="")
    state = _record("state", {"x": 1.0})
    m.advance_window(0.0, 0.5, inputs={"state": state})
    visuals = m.visualize()
    assert visuals[0]["data"]["title"] == "What did the model simulate?"

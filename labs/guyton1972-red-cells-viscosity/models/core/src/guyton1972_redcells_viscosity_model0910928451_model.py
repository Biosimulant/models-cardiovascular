...
"""Auto-generated SBML BioModule wrapper for Guyton1972_RedCells_Viscosity.
...
class SbmlGuyton1972RedcellsViscosity(biosim.BioModule):
    """BioModule wrapper for SBML model: Guyton1972_RedCells_Viscosity."""
    ...
    def __init__(self, model_path: str = "data/MODEL0910928451.xml", integration_step: float = 0.01) -> None:
    ...
    def inputs(self) -> dict[str, SignalSpec]:
        return {
            "plasma_volume": SignalSpec.scalar(
                dtype="float64",
                accepted_profiles=(
                    AcceptedSignalProfile(signal_type="scalar", dtype="float64", accepted_units=("dimensionless",)),
                    AcceptedSignalProfile(signal_type="record", schema={"payload": "json"}),
                ),
                description="Sets plasma volume for the simulation scenario. Default `3.00449`.",
            ),
            "erythropoiesis_drive": SignalSpec.scalar(
                dtype="float64",
                accepted_profiles=(
                    AcceptedSignalProfile(signal_type="scalar", dtype="float64", accepted_units=("dimensionless",)),
                    AcceptedSignalProfile(signal_type="record", schema={"payload": "json"}),
                ),
                description="Sets erythropoiesis drive for the simulation scenario. Default `150.0`.",
            ),

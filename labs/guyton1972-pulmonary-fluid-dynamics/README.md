# Guyton1972 Pulmonary Fluid Dynamics Lab

This lab asks: **Does pulmonary fluid accumulate?**
...
- Pulmonary fluid influx
- Pulmonary lymph flow

The captured run applies the configured pressure-load challenge, changing capillary pressure from `29.9941` to `35.99292` during the challenge window before returning to baseline. It evaluates 600 s of simulated dynamics across Plasma fluid volume, Pulmonary interstitial pressure, Pulmonary fluid influx, Pulmonary lymph flow; read the absolute trajectories, baseline-relative plot, response ranking, and summary table together because the variables use different native scales.
...
1. `guyton1972_pulmonary_fluid_dynamics_core`: SBML simulator. Receives every contextual input port (ion concentrations, voltages, conductances, etc. — see below), drives roadrunner, and emits two outputs: `state` (per-window species/state-variable record) and `species_labels` (a one-time map of `{species_id: human_label}` lifted from the SBML `<species name=...>` attributes).
2. `guyton1972_pulmonary_fluid_dynamics_viz`: visualization sub-model. Consumes both `state` and `species_labels` from the core, accumulates the per-window history, prettifies the labels, and renders the two visuals plus the `run_summary` output. No SBML, no roadrunner — pure presentation.
...
- `models/core/src/guyton1972_pulmonaryfluiddynamics_model0911091440_model.py` is the SBML wrapper (no visualization code).
- `models/core/data/MODEL0911091440.xml` is the original SBML file from BioModels.
...
- `models/viz/src/guyton1972_pulmonary_fluid_dynamics_viz.py` consumes core state + labels and renders timeseries, Q&A table, and the `run_summary` record.
...
|---|---|---|---|
| `capillary_pressure` | Sets capillary pressure for the scenario. | 29.9941 | — |
| `plasma_oncotic_pressure` | Sets plasma oncotic pressure for the scenario. | 71.9719 | — |
...
```bash
biosimulant labs import labs/guyton1972-pulmonary-fluid-dynamics
```
...
- License: `CC0` (from upstream BioModels entry [biomodels_ebi:MODEL0911091440](https://www.ebi.ac.uk/biomodels/MODEL0911091440)).
- This wrapper does not modify the upstream biology. To change rates, initial conditions, or kinetic laws, edit the SBML file in `models/core/data/MODEL0911091440.xml` directly.

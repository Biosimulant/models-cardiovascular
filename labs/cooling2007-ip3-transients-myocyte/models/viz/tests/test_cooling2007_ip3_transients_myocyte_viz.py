...
"""Tests for the Cooling2007 Ip3 Transients Myocyte visualization sub-model. No SBML or
...
from src.cooling2007_ip3_transients_myocyte_viz import Cooling2007Ip3transientsCardiacmyocyteBiomd0000000400ModelViz  # noqa: E402
...
def test_instantiation():
    m = Cooling2007Ip3transientsCardiacmyocyteBiomd0000000400ModelViz()
...
def test_lab_question_threads_through_visuals():
    m = Cooling2007Ip3transientsCardiacmyocyteBiomd0000000400ModelViz(lab_question="Does this experiment work?")
    state = _record("state", {"x": 1.0})
    m.advance_window(0.0, 0.5, inputs={"state": state})
    visuals = m.visualize()
...
def test_generic_title_when_lab_question_blank():
    m = Cooling2007Ip3transientsCardiacmyocyteBiomd0000000400ModelViz(lab_question="")
    state = _record("state", {"x": 1.0})
    m.advance_window(0.0, 0.5, inputs={"state": state})
    visuals = m.visualize()
    assert visuals[0]["data"]["title"] == "What did the model simulate?"

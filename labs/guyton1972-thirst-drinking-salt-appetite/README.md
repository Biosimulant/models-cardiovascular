# Guyton1972 Thirst Drinking Salt Appetite Lab

This lab asks: **Does thirst or salt appetite activate?**
...
1. `guyton1972_thirst_drinking_salt_appetite_core`: SBML simulator. Receives every contextual input port (ion concentrations, voltages, conductances, etc. — see below), drives roadrunner, and emits two outputs: `state` (per-window species/state-variable record) and `species_labels` (a one-time map of `{species_id: human_label}` lifted from the SBML `<species name=...>` attributes).
2. `guyton1972_thirst_drinking_salt_appetite_viz`: visualization sub-model. Consumes both `state` and `species_labels` from the core, accumulates the per-window history, prettifies the labels, and renders the two visuals plus the `run_summary` output. No SBML, no roadrunner — pure presentation.
...
- `models/core/src/guyton1972_thirstdrinking_saltappetite_model0910846879_model.py` is the SBML wrapper (no visualization code).
- `models/core/data/MODEL0910846879.xml` is the original SBML file from BioModels.
...
- `models/viz/src/guyton1972_thirst_drinking_salt_appetite_viz.py` consumes core state + labels and renders timeseries, Q&A table, and the `run_summary` record.
...
|---|---|---|---|
| `antidiuretic_hormone_concentration` | Sets antidiuretic hormone concentration for the scenario. | 1 | — |
| `angiotensin_drive` | Sets angiotensin drive for the scenario. | 0.987545 | — |
| `arterial_pressure` | Sets arterial pressure for the scenario. | 35.1148 | — |
...
```bash
biosimulant labs import labs/guyton1972-thirst-drinking-salt-appetite
```
...
- License: `CC0` (from upstream BioModels entry [biomodels_ebi:MODEL0910846879](https://www.ebi.ac.uk/biomodels/MODEL0910846879)).
- This wrapper does not modify the upstream biology. To change rates, initial conditions, or kinetic laws, edit the SBML file in `models/core/data/MODEL0910846879.xml` directly.
...
| `POT` | POT | 35.1148 |
| `STH` | STH | _(unset)_ |
| `ANMSML` | ANMSML | _(unset)_ |
| `STH1` | STH1 | _(unset)_ |
| `AHCM` | AHCM | _(unset)_ |
| `ANMTH` | ANMTH | _(unset)_ |
| `TVD` | TVD | 0.000980838 |
| `AHTH` | AHTH | _(unset)_ |
| `AHTH1` | AHTH1 | _(unset)_ |
| `TVZ` | TVZ | _(unset)_ |
| `TVZ1` | TVZ1 | _(unset)_ |
| `Z10` | Z10 | 45 |
| `Z11` | Z11 | 0.01 |

...
"""Auto-generated SBML BioModule wrapper for Cooling2007_IP3transients_CardiacMyocyte.
...
class SbmlCooling2007Ip3transientsCardiacmyocyte(biosim.BioModule):
    """BioModule wrapper for SBML model: Cooling2007_IP3transients_CardiacMyocyte."""

    _PORT_TO_SBML: dict[str, str] = {
        "inositol_trisphosphate_initial": "IP3",
        "calcium_initial": "Ca",
        "receptor_initial": "R",
    }
    ...
    def __init__(self, model_path: str = "data/BIOMD0000000400.xml", integration_step: float = 0.01) -> None:
    ...
    def inputs(self) -> dict[str, SignalSpec]:
        return {
            "inositol_trisphosphate_initial": SignalSpec.scalar(
                dtype="float64",
                accepted_profiles=(
                    AcceptedSignalProfile(signal_type="scalar", dtype="float64", accepted_units=("dimensionless",)),
                    AcceptedSignalProfile(signal_type="record", schema={"payload": "json"}),
                ),
                description="Sets inositol trisphosphate initial for the simulation scenario. Default `0.015`.",
            ),
            "calcium_initial": SignalSpec.scalar(
                dtype="float64",
                accepted_profiles=(
                    AcceptedSignalProfile(signal_type="scalar", dtype="float64", accepted_units=("dimensionless",)),
                    AcceptedSignalProfile(signal_type="record", schema={"payload": "json"}),
                ),
                description="Sets calcium initial for the simulation scenario. Default `0.1`.",
            ),
            "receptor_initial": SignalSpec.scalar(
                dtype="float64",
                accepted_profiles=(
                    AcceptedSignalProfile(signal_type="scalar", dtype="float64", accepted_units=("dimensionless",)),
                    AcceptedSignalProfile(signal_type="record", schema={"payload": "json"}),
                ),
                description="Sets receptor initial for the simulation scenario. Default `13.9`.",
            ),

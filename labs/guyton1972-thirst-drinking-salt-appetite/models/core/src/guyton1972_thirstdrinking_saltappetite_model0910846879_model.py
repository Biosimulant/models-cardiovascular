...
"""Auto-generated SBML BioModule wrapper for Guyton1972_ThirstDrinking_SaltAppetite.
...
class SbmlGuyton1972ThirstdrinkingSaltappetite(biosim.BioModule):
    """BioModule wrapper for SBML model: Guyton1972_ThirstDrinking_SaltAppetite."""

    _PORT_TO_SBML: dict[str, str] = {
        "antidiuretic_hormone_concentration": "ADHC",
    ...
    def __init__(self, model_path: str = "data/MODEL0910846879.xml", integration_step: float = 0.01) -> None:
    ...
    def inputs(self) -> dict[str, SignalSpec]:
        return {
            "antidiuretic_hormone_concentration": SignalSpec.scalar(
                dtype="float64",
                accepted_profiles=(
                    AcceptedSignalProfile(signal_type="scalar", dtype="float64", accepted_units=("dimensionless",)),
                    AcceptedSignalProfile(signal_type="record", schema={"payload": "json"}),
                ),
                description="Sets antidiuretic hormone concentration for the simulation scenario. Default `1.0`.",
            ),
            "angiotensin_drive": SignalSpec.scalar(
                dtype="float64",
                accepted_profiles=(
                    AcceptedSignalProfile(signal_type="scalar", dtype="float64", accepted_units=("dimensionless",)),
                    AcceptedSignalProfile(signal_type="record", schema={"payload": "json"}),
                ),
                description="Sets angiotensin drive for the simulation scenario. Default `0.987545`.",
            ),
            "arterial_pressure": SignalSpec.scalar(
                ...
                description="Sets arterial pressure for the simulation scenario. Default `35.1148`.",
            ),
